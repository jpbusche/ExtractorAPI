...
def get_games_db():
	games = []
	response = requests.get('http://game-elt.ml:9200/steam_est/_search?')
	response = json.loads(response.content)
	total = response['hits']['total']
	response = requests.get('http://game-elt.ml:9200/steam_est/_search?size=' + str(total))
	response = json.loads(response.content)
	games = response['hits']['hits']
	for game in response:
